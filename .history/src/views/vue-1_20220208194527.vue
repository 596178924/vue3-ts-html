<template>
	<div class="home">
		...
		<input type="number" :value="data.id" @input="e => (data.id = e.target.value)" />
		...
		<div>
			<p>点击列表切换样式</p>
			<ul>
				<li v-for="item in 3" :key="item" :class="[{ active: classActive == item }]" @click="classActive = item">item = {{ item }}</li>
			</ul>
		</div>
		<div>
			<p>更改触发事件</p>
		...
		<div>
			<p>修饰符</p>
			<form action="*" method="post"><input type="submit" @click.prevent="sayHi" value="click me to submit" /></form>
		</div>
	</div>
</template>
...
let vIf: boolean = ref(true);
let classActive: number = ref(1);
...
let handleEventName: boolean = ref(true);
let computedEventName: string = computed(() => handleEventName.value?'click':'dblclick');
